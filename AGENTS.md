...
- **AI**: System `amp` CLI for execution, validation, and analysis
- **UI**: bits-ui primitives + lucide-svelte icons + Tailwind 4
- **Types**: TypeScript enums mirror Rust enums via serde

#### Amp CLI Integration

Maestro uses the system-installed `amp` CLI for all AI operations:

- **Execution**: Invokes `amp -x "<prompt>" --stream-json` in repository worktrees
- **Validation**: Invokes `amp -x "<validation_prompt>" --stream-json` in the same worktree
- **Analysis**: Invokes `amp -x "<analysis_prompt>" --stream-json` in temporary directories
- **Discovery**: Uses `which::which("amp")` to locate the CLI in PATH
- **Authentication**: Passes `AMP_API_KEY` from keyring via environment variable
- **Streaming**: Parses NDJSON output from stdout for progress updates and session tracking

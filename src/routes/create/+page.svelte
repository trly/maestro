<script lang="ts">
	...
	async function createPromptSet() {
		...
		const repoPromises = selectedRepos.map(async (repo) => {
			try {
				const dbRepo = await api.repositories
					.find(repo.provider, repo.fullName)
					.catch(async (findErr) => {
		...
		try {
			currentPromptSet = await api.promptSets.create(promptSetName, repoIds, null)
	...
	async function saveRevision(executeAfterCreate = true) {
		...
		try {
			...
			const currentRevision = await api.revisions.create(currentPromptSet.id, promptText, parentId)
				...
				{#if selectedRepos.length > 0}
					<UiTooltip content="Number of repositories selected for this prompt set">
						{#snippet children({ props })}
							<span
								{...props}
								class="bg-accent text-accent-foreground px-3 py-1 rounded-full text-sm font-semibold cursor-help"
							>

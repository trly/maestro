...
<div class="@container/app h-screen max-h-screen bg-background flex flex-col">
	<div class="flex-1 flex overflow-hidden min-h-0 max-h-screen">
		<aside class="shrink-0 border-r border-border/20 bg-card overflow-hidden transition-all duration-300 flex flex-col @max-md/app:hidden @max-6xl/app:w-12 @6xl/app:{sidebarCollapsed ? 'w-12' : 'w-72 @lg/app:w-80'}">
			<PromptSidebar collapsed={sidebarCollapsed} onToggleCollapse={() => sidebarCollapsed = !sidebarCollapsed} pathname={data.pathname} searchParams={data.searchParams} />
		</aside>
...
{#if mobileSidebarOpen}
	<Dialog.Root bind:open={mobileSidebarOpen}>
		<Dialog.Portal>
			...
			>
				<PromptSidebar collapsed={false} onToggleCollapse={() => {}} pathname={data.pathname} searchParams={data.searchParams} />
			</Dialog.Content>
		</Dialog.Portal>
	</Dialog.Root>
{/if}
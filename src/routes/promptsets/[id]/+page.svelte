<script lang="ts">
	...
	let executionsWithUpdates = $derived.by(() => {
		...
		return executions.map((execution) => {
			...
			return {
				...
				...(executionUpdates?.validationStatus && {
					validationStatus: executionUpdates.validationStatus,
				}),
				...(executionUpdates?.validationThreadUrl && {
					validationThreadUrl: executionUpdates.validationThreadUrl,
				}),
				...
				...(executionUpdates?.progressMessage && {
					progressMessage: executionUpdates.progressMessage,
				}),
	...
	let revisionStats = $derived(
		revisions.reduce(
			...
			{} as Record<
				string,
				{ total: number; running: number; completed: number; validationPassed: number }
			>
		)
	)
		...
		try {
			...
			if (revisionParam) {
				const revision = revisions.find(
					(r) => r.id === revisionParam || toShortHash(r.id) === revisionParam
				)
				if (revision) {
	...
	async function viewRevisionExecutions(revision: PromptRevision) {
		...
		const updatedExecutions = executions
			.filter((e) => e.revisionId !== revision.id)
	...
	function startPolling(revisionId: string) {
		...
		stopPolling = pollExecutions(revisionId, (execs: Execution[]) => {
			...
			const updatedExecutions = executions.filter((e) => e.revisionId !== revisionId).concat(execs)
			executions = updatedExecutions

			const hasActiveExecution = execs.some(
				(e) =>
					e.status === "pending" ||
					e.status === "running" ||
					e.validationStatus === "pending" ||
	...
	async function backfillMissingStats() {
		const missingStats = executions.filter(
			(e) =>
				e.status === "completed" &&
				e.filesAdded === 0 &&
				e.filesRemoved === 0 &&
	...
	async function executeRevision(revision: PromptRevision) {
		...
		const toStart = existingExecutions.filter((e) => e.status === "pending" && !e.sessionId)
		// Resume: cancelled/failed, or pending with session_id (was started before)
		const toResume = existingExecutions.filter(
			(e) =>
				e.status === "cancelled" || e.status === "failed" || (e.status === "pending" && e.sessionId)
		)
	...
	async function stopExecutionManually(execution: Execution) {
		try {
			...
			executions = executions.map((e) =>
				e.id === execution.id ? { ...e, status: "cancelled" as const } : e
			)
	...
	async function stopValidationManually(execution: Execution) {
		try {
			// Optimistic update
			executions = executions.map((e) =>
				e.id === execution.id ? { ...e, validationStatus: "cancelled" as const } : e
			)
			...
			// Revert optimistic update on error (restore running state)
			executions = executions.map((e) =>
				e.id === execution.id ? { ...e, validationStatus: "running" as const } : e
			)
	...
	async function validateExecutionManually(execution: Execution) {
		try {
			...
			executions = executions.map((e) =>
				e.id === execution.id ? { ...e, validationStatus: null } : e
			)
	...
	async function startExecutionManually(execution: Execution) {
		try {
			...
			executions = executions.map((e) =>
				e.id === execution.id ? { ...e, status: execution.status } : e
			)
	...
	async function refreshAllCiManually() {
		...
		try {
			...
			await Promise.all(revisionExecutions.map((execution) => api.ci.refreshStatus(execution.id)))
	...
	async function bulkDeleteExecutions(selectedExecutions: Execution[]) {
		...
		try {
			...
			if (successCount > 0) {
				showToast(
					`${successCount} execution${successCount > 1 ? "s" : ""} deleted successfully`,
					"success"
				)
			}
			if (failCount > 0) {
		...
		try {
			...
			for (const execution of selectedExecutions) {
				try {
					...
					executions = executions.map((e) =>
						e.id === execution.id ? { ...e, validationStatus: execution.validationStatus } : e
					)
					failCount++
				}
			}
	...
	async function bulkRevalidateExecutions(selectedExecutions: Execution[]) {
		try {
			...
			if (failCount > 0) {
				showToast(
					`Failed to revalidate ${failCount} validation${failCount > 1 ? "s" : ""}`,
					"error"
				)
			}
		} finally {
	...
	async function saveRepositoriesByIds(repoProviderIds: string[]) {
		...
		const repoPromises = repoProviderIds.map(async (providerId) => {
			try {
				const dbRepo = await api.repositories.find("github", providerId).catch(async () => {
		...
		const existingRepoIds = new Set(
			executions.filter((e) => e.revisionId === currentRevision!.id).map((e) => e.repositoryId)
		)
		...
		if (newRepoIds.length > 0) {
			...
			showToast(
				`Repositories updated, ${newRepoIds.length} new execution${newRepoIds.length > 1 ? "s" : ""} created`,
				"success"
			)
		} else {
	...
	async function saveRepositories(repos: ProviderRepository[]) {
		...
		const repoPromises = repos.map(async (repo) => {
			try {
				const dbRepo = await api.repositories.find(repo.provider, repo.fullName).catch(async () => {
	...
	async function handleAnalyzeExecutions(revision: PromptRevision) {
		...
		try {
			...
			showToast(
				`Analyzing ${failedExecutions.length} failed execution${failedExecutions.length > 1 ? "s" : ""}...`,
				"info"
			)
	...
	async function handleAnalyzeValidations(revision: PromptRevision) {
		...
		try {
			...
			showToast(
				`Analyzing ${failedValidations.length} failed validation${failedValidations.length > 1 ? "s" : ""}...`,
				"info"
			)
	...
	onMount(async () => {
		...
		unlistenCommit = await listen<{
			executionId: string
			commitStatus: string
			commitSha?: string
			committedAt?: number | null
		}>("execution:commit", (event) => {
	...
	$effect(() => {
		...
		executionUpdates.forEach((updates, executionId) => {
			...
			if (updates.status === "completed" && !liveStats.has(executionId)) {
				fetchExecutionStats(executionId)
					.then((stats) => {
						liveStats = new Map(liveStats).set(executionId, stats)
					})
					.catch((err) => {
			...
			{:else}
				<div class="flex items-center justify-center h-full">
					<div class="text-center text-muted-foreground max-w-md">
						...
						<p class="text-sm">
							Choose a revision from the sidebar to view its executions and prompt details.
						</p>
					</div>
				</div>
			{/if}
...
<Dialog.Root bind:open={analysisDialogOpen}>
	<Dialog.Portal>
		...
		<Dialog.Content
			class="fixed inset-4 z-50 rounded-lg border border-border bg-background shadow-lg flex flex-col overflow-hidden"
			forceMount={false}
		>
			<div class="overflow-y-auto p-6 pt-16">
				<Dialog.Close
					class="absolute top-4 right-4 z-10 text-muted-foreground hover:text-foreground transition-colors"
					aria-label="Close"
				>
					<X class="w-4 h-4" />
				</Dialog.Close>

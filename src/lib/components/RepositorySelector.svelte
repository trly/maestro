		...
		<div class="flex items-center justify-between gap-2 mb-2">
			{#if selectedRepos.length > 0}
				<span class="text-sm text-muted-foreground"
					...
					{selectedRepos.length === 1 ? "repository" : "repositories"} selected</span
				>
				<button
					onclick={clearAll}
		...
		>
			...
			>
				<div class="max-h-80 overflow-y-auto p-1">
					{#if isSearching}
						<div class="px-5 py-8 text-center text-muted-foreground text-sm">Searching...</div>
						...
						<div class="max-h-80 overflow-y-auto p-1">
							...
							{:else if filteredSgResults.length > 0}
								<div class="p-2 border-b border-border/30">
									...
									>
										Add all {filteredSgResults.length}
										{filteredSgResults.length === 1 ? "repository" : "repositories"}
									</button>
								</div>

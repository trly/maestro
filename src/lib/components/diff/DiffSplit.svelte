...
<div class="grid grid-cols-2 gap-2 rounded-md border border-border/30 overflow-hidden">
	<div class="flex flex-col">
		...
		<div
			...
			class="h-96 bg-background overflow-auto"
		>
			<DiffCodePane>
				{#each items as item}
					{#if item.type === "removed"}
						<div class="bg-destructive/10 text-destructive -mx-4 px-4 flex">
							<span class="text-muted-foreground mr-4 select-none w-12 text-right shrink-0"
								>{item.oldLineNumber || ""}</span
							>

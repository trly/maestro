	...
	{#each items as item}
		...
		{:else if item.type === "modified"}
			<div class="bg-destructive/10 text-destructive">
				- {#each item.segments || [] as seg}{#if seg.removed}<span
							class="bg-destructive/30 font-semibold">{seg.value}</span
						>{:else if !seg.added}{seg.value}{/if}{/each}
			</div>
			<div class="bg-success/10 text-success">
				+ {#each item.segments || [] as seg}{#if seg.added}<span class="bg-success/30 font-semibold"
							>{seg.value}</span
						>{:else if !seg.removed}{seg.value}{/if}{/each}
			</div>
		{:else}

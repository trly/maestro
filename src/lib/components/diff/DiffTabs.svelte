...
<Tabs.Root bind:value={viewMode}>
	<div class="mb-3 flex items-center justify-between">
		<Tabs.List class="inline-flex rounded-sm bg-muted p-1">
			<Tabs.Trigger
				value="split"
				class="px-3 py-1.5 text-sm font-medium rounded-sm transition-all data-[state=active]:bg-background data-[state=active]:shadow-sm"
			>
				Split
			</Tabs.Trigger>
			<Tabs.Trigger
	...
	<Tabs.Content value="unified">
		<div class="rounded-md border border-border/30 overflow-hidden">
			...
			<div bind:this={scrollElement} {onscroll} class="h-96 bg-muted/30 overflow-auto opacity-75">
				<DiffUnified {items} />
			</div>
		</div>
	</Tabs.Content>

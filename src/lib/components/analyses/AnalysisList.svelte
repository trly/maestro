...
<div class="flex flex-col min-h-0 overflow-hidden">
	...
	>
		...
		<!-- Prompts -->
		<div class="text-left text-xs font-semibold text-muted-foreground">Prompts</div>
		...
		<!-- Actions -->
		<div class="text-left text-xs font-semibold text-muted-foreground">Actions</div>
	</div>

	<!-- Body -->
	<div class="flex-1 overflow-y-auto">
		{#if props.ids.length === 0}
			<div class="px-4 py-8 text-center text-sm text-muted-foreground">No analyses found</div>
		{:else}

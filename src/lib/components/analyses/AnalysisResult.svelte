<script lang="ts">
	...
	let formattedResult = $derived(
		reactiveAnalysis.analysisResult ? (marked.parse(reactiveAnalysis.analysisResult) as string) : ""
	)
...
<div class="space-y-6">
	<div class="flex items-start justify-between gap-4">
		<div class="flex-1 space-y-2">
			<div class="flex items-center gap-2">
				...
				<span
					class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md {statusConfig.bgClass} {statusConfig.textClass} text-xs font-medium"
				>
	...
	{#if reactiveAnalysis.analysisResult}
		<div
			class="prose prose-sm max-w-none text-foreground prose-headings:text-foreground prose-p:text-foreground prose-strong:text-foreground prose-code:text-foreground"
		>

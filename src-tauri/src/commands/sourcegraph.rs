...
use crate::sourcegraph::{SourcegraphClient, RepositorySearchResult};
...
pub async fn search_sourcegraph_repositories(
	...
	let endpoint = tokens::get_token_value("sourcegraph_endpoint")
		...
		.ok_or_else(|| "Sourcegraph endpoint not configured. Please set it in Settings.".to_string())?;
	...
	let access_token = tokens::get_token_value("sourcegraph_token")
		...
		.ok_or_else(|| "Sourcegraph access token not configured. Please set it in Settings.".to_string())?;

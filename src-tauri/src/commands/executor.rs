...
async fn add_worktree(
	...
	if !output.status.success() {
		anyhow::bail!("Failed to create worktree: {}", String::from_utf8_lossy(&output.stderr));
	}
...
async fn remove_worktree(
	...
	if !output.status.success() {
		log::error!("[remove_worktree] Failed to remove worktree: {}", String::from_utf8_lossy(&output.stderr));
	}
...
async fn execute_with_amp(
	...
	let bun_path = which::which("bun")
		.context("bun command not found in PATH")?;
	...
	let stderr = child.lock().unwrap().stderr.take().context("Failed to capture stderr")?;
	let stdout = child.lock().unwrap().stdout.take().context("Failed to capture stdout")?;
	...
	let status = child.lock().unwrap().wait().context("Failed to wait for bun process")?;
...
pub(crate) async fn execute_prompt_impl(
	...
	let (execution, repository, revision) = {
		...
		let repository = store.get_repository(&execution.repository_id)?
			.ok_or_else(|| anyhow::anyhow!("Repository {} not found", execution.repository_id))?;
		let revision = store.get_prompt_revision(&execution.revision_id)?
...
PROMPT: FAIL";
	...
	match result {
		...
		Err(e) => {
			...
			let status = if is_aborted { ExecutionStatus::Cancelled } else { ExecutionStatus::Failed };
...
pub async fn validate_execution(
	...
	validate_execution_impl(execution_id, app, paths.inner().clone()).await.map_err(|e| e.to_string())
}
...
async fn validate_execution_impl(
	...
	// Guard against duplicate validation runs
	{
		...
		if active.contains_key(&execution_id) {
			anyhow::bail!("Validation for execution {} is already running", execution_id);
		}
		if execution.validation_status == Some(ValidationStatus::Running) {
			anyhow::bail!("Validation for execution {} is already in running state", execution_id);
		}
	}
...
VALIDATION: FAIL";
	...
	match result {
		Ok(_) => Ok(()),
		Err(e) => {
			log::error!("[validate_execution] Validation {} failed: {}", execution_id, e);
			let is_aborted = e.to_string().contains("aborted");
			let status = if is_aborted { ValidationStatus::Cancelled } else { ValidationStatus::Failed };
			let status_str = if is_aborted { "cancelled" } else { "failed" };
...
pub async fn resume_execution(
	...
	resume_execution_impl(execution_id, app, paths.inner().clone()).await.map_err(|e| e.to_string())
}
...
async fn resume_execution_impl(
	...
	let (execution, repository) = {
		...
		let _ = store.get_prompt_revision(&execution.revision_id)?
			.ok_or_else(|| anyhow::anyhow!("Revision {} not found", execution.revision_id))?;
...
PROMPT: FAIL";
	...
	match result {
		Ok(_) => Ok(()),
		Err(e) => {
			log::error!("[resume_execution] Execution {} failed: {}", execution_id, e);
			let is_aborted = e.to_string().contains("aborted");
			let status = if is_aborted { ExecutionStatus::Cancelled } else { ExecutionStatus::Failed };
			let status_str = if is_aborted { "cancelled" } else { "failed" };
...
pub async fn commit_changes(
	...
	let (session_id, promptset_id, _repository_id) = {
		...
		if execution.session_id.is_none() {
			return Err(format!("Cannot commit {} - no session ID found", execution_id));
		}
		(
	...
	let commit_prompt = if let Some(files) = &files {
		...
		format!("Please commit only the following files with an appropriate commit message:\n{}", file_list)
	} else {
	...
	
	{
		...
		if has_uncommitted {
			...
			let committed_at = GitService::get_commit_timestamp(&repo, "HEAD").map_err(|e| e.to_string())?;
			let parent_sha = GitService::get_parent_sha(&repo, "HEAD").map_err(|e| e.to_string())?;
			...
			emit_execution_commit(&app, &execution_id, "committed", Some(&commit_sha), Some(&committed_at));
		}
	}
...
pub fn reconcile_on_startup(store: &Store) -> Result<()> {
	...
	for execution in executions {
		...
		if execution.validation_status == Some(ValidationStatus::Running) && !active_vals.contains_key(&execution.id) {
			store.update_execution(
				&execution.id,
				ExecutionUpdates {
					validation_status: Some(ValidationStatus::Cancelled),
...
pub async fn cleanup_execution(
	...
	let (repository, promptset_id, branch) = {
		...
		(repository, execution.promptset_id.clone(), execution.branch.clone())
	};
...
pub fn get_execution_modified_files(
		...
		&& execution.parent_sha.is_some()
	{
		...
		let (owner, repo_name) = parse_provider_id(&repository.provider_id).map_err(|e| e.to_string())?;

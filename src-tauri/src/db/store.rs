...
use crate::types::{
    Analysis, AnalysisStatus, AnalysisType, CiStatus, CommitStatus, ExecutionStatus, PromptStatus,
    ValidationStatus,
};
...
impl Store {
    ...
    pub fn get_repository(&self, id: &str) -> Result<Option<Repository>> {
        let mut stmt = self
            .conn
            .prepare_cached(&format!("{SELECT_REPOSITORY} WHERE id = ?1"))?;
    ...
    pub fn find_repository(&self, provider: &str, provider_id: &str) -> Result<Option<Repository>> {
        let mut stmt = self.conn.prepare_cached(&format!(
            "{SELECT_REPOSITORY} WHERE provider = ?1 AND provider_id = ?2"
        ))?;
    ...
    pub fn get_all_repositories(&self) -> Result<Vec<Repository>> {
        let mut stmt = self
            .conn
            .prepare(&format!("{SELECT_REPOSITORY} ORDER BY created_at DESC"))?;
        let repos = stmt
    ...
    pub fn update_promptset_validation(
        &self,
        id: &str,
        validation_prompt: Option<String>,
    ) -> Result<()> {
    ...
    pub fn update_promptset_repositories(
        &mut self,
        id: &str,
        repository_ids: Vec<String>,
    ) -> Result<()> {
        ...
        tx.execute(
            "DELETE FROM promptset_repositories WHERE promptset_id = ?1",
            [id],
        )?;
    ...
    pub fn get_prompt_revision(&self, id: &str) -> Result<Option<PromptRevision>> {
        let mut stmt = self
            .conn
            .prepare_cached("SELECT * FROM prompt_revisions WHERE id = ?1")?;
    ...
    pub fn find_prompt_revision_by_prefix(
        &self,
        id_prefix: &str,
        ...
        let mut stmt = self
            .conn
            .prepare_cached("SELECT * FROM prompt_revisions WHERE id LIKE ?1")?;
    ...
    pub fn get_execution(&self, id: &str) -> Result<Option<Execution>> {
        let mut stmt = self
            .conn
            .prepare_cached(&format!("{SELECT_EXECUTION} WHERE id = ?1"))?;
    ...
    pub fn find_execution_by_prefix(&self, id_prefix: &str) -> Result<Option<Execution>> {
        ...
        let mut stmt = self
            .conn
            .prepare_cached(&format!("{SELECT_EXECUTION} WHERE id LIKE ?1"))?;
    ...
    pub fn get_executions_by_revision(&self, revision_id: &str) -> Result<Vec<Execution>> {
        ...
        let executions = stmt
            .query_map([revision_id], map_execution)?
            .collect::<rusqlite::Result<Vec<_>>>()?;
        Ok(executions)
    }
    ...
    pub fn get_executions_by_promptset(&self, promptset_id: &str) -> Result<Vec<Execution>> {
        ...
        let executions = stmt
            .query_map([promptset_id], map_execution)?
            .collect::<rusqlite::Result<Vec<_>>>()?;
        Ok(executions)
    }
    ...
    pub fn get_all_executions(&self) -> Result<Vec<Execution>> {
        let mut stmt = self
            .conn
            .prepare(&format!("{SELECT_EXECUTION} ORDER BY created_at DESC"))?;
        let executions = stmt
            .query_map([], map_execution)?
            .collect::<rusqlite::Result<Vec<_>>>()?;
        Ok(executions)
    }
    ...
    pub fn delete_execution(&self, id: &str) -> Result<bool> {
        let result = self
            .conn
            .execute("DELETE FROM executions WHERE id = ?1", [id])?;
        Ok(result > 0)
    }
    ...
    pub fn delete_promptset(&self, id: &str) -> Result<bool> {
        let result = self
            .conn
            .execute("DELETE FROM promptsets WHERE id = ?1", [id])?;
        Ok(result > 0)
    }
    ...
    pub fn get_ci_stuck_threshold_minutes(&self) -> Result<i64> {
        let value = self
            .get_setting("ci_stuck_threshold_minutes")?
            .unwrap_or_else(|| "10".to_string());
        value
            .parse::<i64>()
            .map_err(|e| anyhow::anyhow!("Invalid CI threshold: {}", e))
    }

    pub fn get_max_concurrent_executions(&self) -> Result<i64> {
        let value = self
            .get_setting("max_concurrent_executions")?
            .unwrap_or_else(|| "10".to_string());
        value
            .parse::<i64>()
            .map_err(|e| anyhow::anyhow!("Invalid max concurrent executions: {}", e))
    }
    ...
    pub fn get_analysis(&self, id: &str) -> Result<Option<Analysis>> {
        let mut stmt = self
            .conn
            .prepare_cached(&format!("{SELECT_ANALYSIS} WHERE a.id = ?1 GROUP BY a.id"))?;
    ...
    pub fn update_analysis_prompt(&self, id: &str, analysis_prompt: &str) -> Result<()> {
        let now = now_ms();
        self.conn.execute(
    ...
    pub fn get_analysis_executions(&self, analysis_id: &str) -> Result<Vec<Execution>> {
        ...
        let executions = stmt
            .query_map([analysis_id], map_execution)?
            .collect::<rusqlite::Result<Vec<_>>>()?;
        Ok(executions)
    }
